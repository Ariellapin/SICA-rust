...
| **UI-8 Workspaces, onboarding, integrations** ⏳ *(§4.3 done)* | ~~Workspace grouping in the sidebar and Add workspace over `rfd` (§4.3)~~ · ~~the hero picker and the session's workspace as the header crumb (§4.3, §8)~~ · ~~the first-run onboarding modal and "key missing" badges (§7.3)~~ · ~~Settings › Agents and Settings › Integrations (§7.2)~~ · ~~the lightbox and history sizing (§5.3)~~; file cards need harness §9.6 · ~~markdown extras (§3.8)~~ · ~~`@session` (§6.12)~~ · the workflow run body (§6.11 — ~~interim phase list done~~; the tree still needs the durable event) | **L** | v26 (harness Wave 9) |
