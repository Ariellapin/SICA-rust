...
| `Event::QueueChanged { session_id, rows: Vec<QueuedDump> }` · `Request::EditQueued / RemoveQueued / SteerQueued { id, … }` | queue dock §5 | open |
...
| **UI-3 Composer + control plane** ✅ | r=22 card, toolbar (`+`, permission chip + risk gate, plan chip, model select, context ring with the `TokenBreakdown` panel, send/stop), keymap with the busy-Enter preference, dock (to-dos, goal, queue), stats line, approval and question/plan-review takeovers, `/` menu with dsh's fuzzy ranking, drop overlay, toasts for WARN/ERROR. **Open:** the queue dock mirrors the FE's own sends until `QueueChanged` exists; `@file` completion and the ghost hint after a claimed command are not in; the `/` menu renders in the composer's panel rather than a floating overlay. | **L** | v17 batch 2 |
