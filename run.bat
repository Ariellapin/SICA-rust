@echo off
rem Wrapper that sets up PATH for cargo + the LLVM-MinGW toolchain, then forwards
rem arguments to cargo.
rem
rem Examples:
rem   run.bat build --workspace
rem   run.bat test --workspace
rem   run.bat run -p frontend
rem   run.bat run -p frontend --bin smoke
